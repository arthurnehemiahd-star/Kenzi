...
# ----------------------------------------------------------------------
# Tool definitions (Anthropic shape; converted in llm.py)
# ----------------------------------------------------------------------
TOOLS = [
    {
        "name": "remember_note",
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "note": {"type": "string", "description": "The note to remember, in plain words."}
            },
            "required": ["note"],
        },
    },
        ...
        "input_schema": {
            ...
            "properties": {
                "url": {
                    "type": "string",
                    "description": (
                        "The URL or URL scheme to open, e.g. 'tel:+15551234567', "
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "Song, artist, or search terms."}
            },
            "required": ["query"],
        },
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "Song and/or artist to search for and save."}
            },
            "required": ["query"],
        },
    },
...
def get_tools_for_platform(platform):
    ...
    rather than trusting the prompt to suppress it."""
    if platform == "ios":
        return TOOLS
    return [t for t in TOOLS if t["name"] != "run_shortcut"]
...
# may run several tools against it, then saves once at the end. If these
# did an independent save here, that final save would silently overwrite
# it with the stale copy.
# ----------------------------------------------------------------------
def get_spotify_access_token(db):
    """Return a valid access token, refreshing if needed. Mutates
    db['spotify'] in place on refresh. Returns None if Spotify has never
    been connected."""
    import time
    ...
    if tokens.get("access_token") and tokens.get("expires_at", 0) > time.time() + 30:
...
def execute_tool(tool_name, tool_input, db):
    """Run one tool call. Returns (result_text_for_model, action_or_None).
    ...
    if tool_name == "remember_note":
        ...
        db.setdefault("notes", []).append(note)
        return "Saved.", None
    ...
    if tool_name == "list_music":
        if not os.path.isdir(MUSIC_DIR):
            return "No music library folder found yet.", None
        tracks = [
            f for f in os.listdir(MUSIC_DIR)
            if os.path.splitext(f)[1].lower() in ALLOWED_AUDIO_EXTENSIONS
        ]
        if not tracks:
            return "The music library is empty.", None
        return "\n".join(f"- {t}" for t in sorted(tracks)), None

    if tool_name == "run_shortcut":
        name = tool_input.get("shortcut_name", "")
        if not name:
            return "No shortcut name given.", None
        scheme_url = f"shortcuts://run-shortcut?name={quote(name)}"
        return f"Running the '{name}' shortcut now.", {"type": "open_url", "url": scheme_url}

    if tool_name == "open_app":
        url = tool_input.get("url", "")
        if not url:
            return "No URL given.", None
        return "Opening that now.", {"type": "open_url", "url": url}

    if tool_name == "open_camera":
        return "Opening the camera now.", {"type": "open_camera"}

    if tool_name == "search_music":
        query = tool_input.get("query", "")
        try:
            items = spotify_search_tracks(db, query, limit=5)
        except requests.RequestException as e:
            return f"Spotify search failed: {e}", None
        if items is None:
            return (
                "Spotify isn't connected yet — she needs to connect it once.",
                {"type": "open_url", "url": "/spotify/login"},
            )
        if not items:
    ...
    if tool_name == "save_music":
        query = tool_input.get("query", "")
        try:
            items = spotify_search_tracks(db, query, limit=1)
        except requests.RequestException as e:
            return f"Spotify search failed: {e}", None
        if items is None:
            return (
        ...
        if not items:
            return f"Couldn't find '{query}' on Spotify.", None
        ...
        try:
            ...
            return f"Couldn't save to Spotify: {e}", None

        artist_names = ", ".join(a["name"] for a in track["artists"])
        return f"Saved '{track['name']}' by {artist_names} to your Spotify library.", None
