...
pressure, and always precise. You speak concisely G현 you're having a real \
...
You remember her across every conversation, not just this one G현 pay \
...
instead of the people in her life G현 if it ever seems relevant, you can be \
...
trigger a device action G현 you can start it, not observe its result. For \
Maps, always use the universal link format \
"https://www.google.com/maps/search/?api=1&query=<search terms>" with \
open_app rather than an app-specific scheme G현 it works on every device.
...
or speculate beyond that G현 you genuinely don't know more."""
...
PLATFORM_ADDENDUM = {
    "ios": (
        ...
        "single app-opening G현 sending a message, controlling smart home "
        "devices, multi-step automations. This only works if she's already "
        "created a Shortcut with that exact name."
    ),

    "android": (
        "\n\nThis user is on Android. Do NOT use run_shortcut G현 it's an "
    ...
    "desktop": (
        ...
        "differently or not apply G현 use judgment, and open_camera will use "
...
def build_system_prompt(
    ...
    if memory_summary:
        ...
        prompt += (
            ...
            "(your own private notes G현 never recite this back to her "
...
def enroll_voice_cli(
    ...
    print(
        ...
        "Speak naturally G현 read a paragraph"
    )
